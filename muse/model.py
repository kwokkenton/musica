...
class MusicTranscriber(nn.Module):
    def __init__(self, model_dict):
        ...
        self.encoder_pos_encodings = nn.Parameter(torch.empty(1, self.enc_max_len, self.d_model))
        nn.init.normal_(self.encoder_pos_encodings, std=0.02)
        self.decoder_pos_embeddings = nn.Parameter(torch.empty(1, self.dec_max_len, self.d_model))
